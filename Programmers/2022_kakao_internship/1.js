...
function solution(survey, choices) {
  ...
  for (let i = 0; i < choices.length; i++) {
    if (choices[i] < 4) {
      char_score[survey[i].split('')[0]] += score_dict[choices[i]];
    } else {
      char_score[survey[i].split('')[1]] += score_dict[choices[i]];
    }
  }

  answer += char_score['T'] > char_score['R'] ? 'T' : 'R';
  answer += char_score['F'] > char_score['C'] ? 'F' : 'C';
  answer += char_score['M'] > char_score['J'] ? 'M' : 'J';
  answer += char_score['N'] > char_score['A'] ? 'N' : 'A';
