...
food = {}
for ing in ings:
    ingredient[ing.split(' ')[0]] = int(ing.split(' ')[1])

for foodInfo in menu:
    tmp = foodInfo.split(' ')
ingcost = 0
costTmp = list(tmp[1])
for c in costTmp:
    ingcost += ingredient[c] #재료비
food[tmp[0]] = int(tmp[2]) - ingcost #판매가 - 재료비 = 수익

for sellInfo in sell:
    revenue += food[sellInfo.split(' ')[0]] * int(sellInfo.split(' ')[1])
